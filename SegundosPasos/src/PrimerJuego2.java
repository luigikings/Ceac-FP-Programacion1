...
public class PrimerJuego2 {
	...
	public static int volverajugar () {
		...
		while (true) {
			...
			try {
				...
				System.err.println("NO HAS INTRODUCIDO UN NÚMERO "+e);
				sc.nextLine();
				continue;
			}
			break;
	...
	public static void adivinanza (int num1,int numadivinar,int intentos) {
		if (num1 >= 0 && num1 <= 100) {
			if (num1 > numadivinar) {
				...
				System.out.println("Felicidades! adivinaste el número en "+intentos+" intentos y era:"+numadivinar);
			}
		} else {
			System.out.println("El número introducido esta fuera de RANGO (1-100)");
		}
	...
	public static void intentaradivinar(int num1,int numadivinar, int intentos) {
		...
		System.out.println("Adivina el número! Introduce un número del 1 al 100");
		...
		//metemos una condicional en donde terminaremos el bucle en caso de que el usuario lo intente 100 veces y no logre adivinar 
			if (intentos == 100) {
				System.out.println("No has podido adivinar el número!");
			}
