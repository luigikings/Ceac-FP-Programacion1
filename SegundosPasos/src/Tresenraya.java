...
public class Tresenraya {
	...
	public static void main(String[] args) {
		...
		// Hacemos un bucle infinito que solo saldra en caso de que gane algun jugador o
		// no haya mas movimientos posibles
	...
	public static void turnoUsuario(char[][] tablero) {
		...
		// Usamos un try ya que queremos que el usuario unicamente introduzca un numero
			try {
				// pedimos el numero y le restamos 1 (ya que las array empiezan en el 0)
		...
		// Metemos una condicional para que si el numero entra dentro del rango podamos
		// escribirlo dentro del tablero, en caso de que no volver a pedir las
		// coordenadas

		if ((x >= 1 && x <= 3) && (y >= 1 && y <= 3)) {

			// y ahora metemos otra condicional para verificar que la posicion deseada esta
			// vacia, esta vacia en caso de que ponga (-), en caso de que no avisar al
			// usuario y volverle a pedir las coordenadas
	...
	// En esta función hacemos que el ordenador juegue su turno eligiendo el numero
	// al azar dentro del rango (0-1-2) si esta ocupada volver a pedir que pruebe
	// otra
	...
	// Esta funcion devuelve un boolean para ver si la partida ya terminado en caso
	// de victoria de algun jugador o que no es posible hacer mas jugadas
	...
	public static boolean comprobarTablero(char[][] tablero) {
		...
		// en este bucle verificamos si hay un espacio en el tablero en caso de que
		// encuentre 1 que devuelva false y siga el juego
		for (int y = 0; y < tablero.length; y++) {
			for (int x = 0; x < tablero[y].length; x++) {
				if (tablero[y][x] == '-') {
					return false;
		...
		// despues de verificar que no hay espacios entonces devolvemos que hubo un
		// empate y termina la partida devolviendo un true
		System.out.println("TENEMOS UN EMPATE:");
		return true;
	}

	// en esta función devuelve un boolean de true si hay ganador o false si no lo
